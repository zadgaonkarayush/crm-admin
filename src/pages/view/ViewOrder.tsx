...
const ViewOrder = () => {
  ...
  const totalTax = order.lines.reduce((sum, item) => sum + item.tax, 0);
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      ...
      showSuccess(`Status changed to ${updated.status} successfully!`);

      navigate('/orders')
    } catch (error) {
      console.error('Failed to update status', error);
      showError('Failed to change status. Please try again later!');
    } finally {
      setLoading(false);
    }
  };
